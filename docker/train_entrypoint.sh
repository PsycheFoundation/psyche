...
while true; do
    ...
    # Exit code 11 means authorization account not initialized - exit container immediately (no retry)
    if [[ $EXIT_STATUS -eq 11 ]]; then
        echo -e "[!] Authorization account not initialized. Exiting container without retry..."
        exit 11
    fi
