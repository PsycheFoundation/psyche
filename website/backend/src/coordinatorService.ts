import { CoordinatorConfig, Model, PsycheCoordinator, RunMetadata, Witness } from "psyche-deserialize-zerocopy-wasm";
import { ChainTimestamp, RunSummary, RunData, TxSummary, getRunPDA, OverTime, Metrics, RunRoundClient } from "shared";
import { runKey, UniqueRunKey } from "./coordinator.js";
import { CoordinatorDataStore, LastUpdateInfo } from "./dataStore.js";
import { WitnessEvalResult, WitnessMetadata } from "./idlTypes.js";
import { decodeBurnCheckedInstruction } from "@solana/spl-token";
...
class CoordinatorService implements CoordinatorDataStore {
    eventEmitter: import("events") <{ update: [UniqueRunKey]; }>;
    #datasStore: CoordinatorDataStore;
    #programId: PublicKey;
    ...
    constructor(dataStore: CoordinatorDataStore, programId: PublicKey) {
        this.#datasStore = dataStore;
        this.eventEmitter = this.#datasStore.eventEmitter;
        this.#programId = programId;
    }

    createRun(pubkey: string, runId: string, timestamp: ChainTimestamp, newState?: PsycheCoordinator): void {
        ...
        this.#datasStore.createRun(pubkey, runId, timestamp, newState);
    }

    updateRun(pubkey: string, newState: PsycheCoordinator, timestamp: ChainTimestamp, configChanged: boolean): void {
        // TODO update in-memory

        this.#datasStore.updateRun(pubkey, newState, timestamp, configChanged);
    }

    setRunPaused(pubkey: string, paused: boolean, timestamp: ChainTimestamp): void {
        const [lastRun, _index] = this.#getActiveRun(pubkey);
        ...
        this.#datasStore.setRunPaused(pubkey, paused, timestamp);
    }

    witnessRun(pubkey: string, witness: WitnessMetadata, timestamp: ChainTimestamp): void {
        const [lastRun, _index] = this.#getActiveRun(pubkey)
        lastRun.lastUpdated = timestamp
        ...
        // TODO this does not typecheck so figure that out
        //lastRun.witnessUpdates.push([
        //	{ ...restWitness, evals: fixedEvals },
        //	timestamp,
        //]);

        this.#datasStore.witnessRun(pubkey, witness, timestamp);
    }
    ...
    destroyRun(pubkey: string, timestamp: ChainTimestamp): void {
        ...
        this.#datasStore.destroyRun(pubkey, timestamp);
    }

    trackTx(runPubkey: string, userPubkey: string, method: string, data: string, txHash: string, timestamp: ChainTimestamp): void {
        ...
        this.#datasStore.trackTx(runPubkey, userPubkey, method, data, txHash, timestamp);
    }
