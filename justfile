...
# Run a single integration test (subprocess mode — no Docker needed).
...
stop_test_infra:
    ...
    pkill -f psyche-solana-client 2>/dev/null || true
    echo "Cleaned up leftover test processes"

# Legacy: run test infra with Docker (for local dev with docker-compose)
run_test_infra_docker num_clients="1":
    #!/usr/bin/env bash
    set -e
    cd docker/test
