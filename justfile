...
run_docker_client *ARGS:
    docker run -d {{ ARGS }} --gpus all nousresearch/psyche-solana-client:latest
...
setup_gpu_clients num_clients="1":
    ...
    just nix build_docker_test_client
...
docker_push_centralized_client:
    just nix docker-build-centralized-client
...
setup_test_infra num_clients="1":
    ...
    just --justfile {{ NIX_JUSTFILE }} build-docker-test-client
    just --justfile {{ NIX_JUSTFILE }} build-docker-solana-test-vaidator
...
setup_test_infra_with_proxies_validator num_clients="1":
    cd architectures/decentralized/solana-coordinator && anchor keys sync && anchor build --no-idl
    cd architectures/decentralized/solana-authorizer && anchor keys sync && anchor build --no-idl
    just nix build-docker-test-client
    just nix build-docker-solana-test-vaidator-client
...
setup_test_infra_three_clients:
    ...
    cd architectures/decentralized/solana-authorizer && anchor keys sync && anchor build --no-idl
    just nix build_docker_test_client
    just nix build_docker_solana_test_vaidator_client
