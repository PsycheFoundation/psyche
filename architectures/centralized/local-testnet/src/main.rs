...
fn start_client(
    ...
    let metrics_local_bind = format!("127.0.0.1:{}", 6269 + i - 1);

    cmd.push(format!(
        "TOKIO_CONSOLE_BIND=127.0.0.1:{tokio_console_port} METRICS_LOCAL_BIND={metrics_local_bind} RUST_LOG={} RUST_BACKTRACE=1 RAW_IDENTITY_SECRET_KEY={} cargo run -p psyche-centralized-client train --run-id {} --server-addr localhost:{} --logs {}",
        args.log,
        raw_key,
        run_id,
        args.server_port,
