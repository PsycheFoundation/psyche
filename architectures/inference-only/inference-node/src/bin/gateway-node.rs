...
>>>>>>> dcb24378 (Addressing PR feedback: adding enums for source type, removing unecessary async calls, and removing manual drops, gateway node changes to allow for model assignment by node and tracking, Justfile updates for model assignment testing)
