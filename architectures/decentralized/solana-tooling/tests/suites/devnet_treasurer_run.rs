...
pub async fn run() {
    ...
    let payer =
        Keypair::from_seed(b"Hello Now this is me lol payerpayerpayerdu")
            .unwrap();

    // Constants
    let main_authority =
        Keypair::from_seed(b"I am the main authority of this run lolol")
            .unwrap();
    let join_authority =
        Pubkey::from_str("9Sj5tX5C5VbkWhkrCvp8ttdCaTbH3AEuez6Hw4do4jFN")
            .unwrap();
    let client = Keypair::new();
    let ticker = Keypair::new();
    let warmup_time = 1;
    let round_witness_time = 1;
    let cooldown_time = 1;
    let rounds_per_epoch = 4;
    let earned_point_per_epoch = 33;

    // Prepare the collateral mint
    let collateral_mint_authority = &main_authority;
    ...
    let authorization =
        Pubkey::from_str("BChTFf9crBeTHiRs17VoWe6PPQn4pg9eXV8ivyBwdd7S")
            .unwrap();
