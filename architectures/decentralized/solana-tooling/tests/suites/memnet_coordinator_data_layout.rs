...
#[tokio::test]
pub async fn run() {
    let coordinator_bytes = include_bytes!("../fixtures/coordinator-account-v1.so").to_vec();
    let coordinator_account = coordinator_account_from_bytes(&coordinator_bytes).unwrap();
    assert_eq!(
