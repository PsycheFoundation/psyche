        ...
        Some(Commands::Config { config, local }) => {
            ...
            let model_name = match &llm.checkpoint {
                Checkpoint::Hub(repo) | Checkpoint::P2P(repo) | Checkpoint::Dummy(repo) => {
                    ...
                    // Resolve relative local paths against the config file's parent dir
                    // Local paths start with "." or "/"; HF slugs are like "owner/name".
                    if repo_id.starts_with('.') {
                        let config_dir = Path::new(&config).parent().unwrap_or(Path::new(""));
                        config_dir.join(&repo_id).to_string_lossy().into_owned()
                    } else {
                        repo_id
                    }
                }
