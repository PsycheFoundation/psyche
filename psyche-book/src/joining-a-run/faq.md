...
- Do I need to know the RUN_ID to join a training run?
  - No. If you don't specify a `RUN_ID` in your `.env` file, the client will automatically discover available runs from the coordinator and join the first active run. Alternatively, you can specify a specific `RUN_ID` if you want to join a particular run.
