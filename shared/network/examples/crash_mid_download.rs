...
use iroh::{NodeAddr, PublicKey, RelayMap, RelayMode, RelayUrl};
    ...
    async fn on_network_event(&mut self, event: NetworkEvent<Message, DistroResultBlob>) {
        match event {
            ...
            NetworkEvent::MessageReceived((from, Message::DistroResult { step, blob_ticket })) => {
                ...
                if self.should_wait {
                    info!("Waiting to download, we will ask to: {peers:?}");
                    tokio::time::sleep(Duration::from_secs(15)).await;
                    info!("Now we download");
                }

                self.network
                    .start_download(blob_ticket, step, &peers)
                    .await
                    .unwrap();
            }
            NetworkEvent::DownloadComplete(result) => {
                ...
                info!(
                    "Download complete: {hash}! step {}: {} downloaded @ {}/s from: {}",
                    file.step,
...
async fn main() -> Result<()> {
    ...
    const SEND_DATA_INTERVAL: u64 = 200;
    ...
    let our_id = network.get_all_peers().await[0].0.clone();
    println!("OUR NODE: {our_id:?}");
    App {
        cancel,
        current_step: 0,
        network,
