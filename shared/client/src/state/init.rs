...
use psyche_data_provider::{
    ...
    download_model_from_gcs_signed_async, download_model_repo_async, fetch_json_from_gcs,
    fetch_json_from_hub,
                                ...
                                CheckpointData::Gcs { .. } => {
                                    let run_down = run_down_client.as_ref().ok_or_else(|| {
                                        DownloadError::RunDown(
                                            "RunDownClient not configured".into(),
                                        )
                                    })?;
                                    info!(
                                    ...
                                    event!(warmup::CheckpointDownloadStarted { size_bytes: 0 });
                                    let repo_files = match download_model_from_gcs_signed_async(
                                        run_down,
                                    )
                                    .await
                                    {
