                                ...
                                match message {
                                    ...
                                    NetworkEvent::DownloadFailed(dl) => {
                                        ...
                                        if retries >= MAX_DOWNLOAD_RETRIES {
                                            warn!("Download failed for blob with hash {hash} (not retrying): {}", dl.error);
                                            ...
                                            let peer_ids = participating_node_ids(&run.coordinator_state().unwrap());
                                            ...
                                            tokio::spawn(async move {
                                                println!("ASKING FOR BLOB TICKET AGAIN");
                                                let blob_ticket_to_retry = if let DownloadType::ModelSharing(request_type) = dl.download_type.clone() {
                                                    let new_blob_ticket = get_blob_ticket_to_download(router, &param_requests_cancel_token, request_type.clone(), peer_cycle, errored_peers, peer_ids.len()).await.unwrap();
                                                ...
                                                println!("BLOB TICKET ADDED FOR RETRY");
