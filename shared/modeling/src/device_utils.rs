...
use tch::{Device, Kind, Tensor, utils::has_mps};
use thiserror::Error;
...
impl Devices {
    ...
    /// Ensures the device is usable by running a quick smoke test on it.
    ///
...
/// Ensures that a CUDA device is usable by running a quick smoke test on it
///
