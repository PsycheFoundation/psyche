...
#[derive(Clone)]
pub enum PretrainedSource<T> {
    RepoFiles(Vec<PathBuf>),
    ConfigAndTensors(T, Arc<HashMap<String, Tensor>>),
}

unsafe impl<T> Send for PretrainedSource<T> {}
unsafe impl<T> Sync for PretrainedSource<T> {}

impl<T: Clone + serde::de::DeserializeOwned> PretrainedSource<T> {
...
impl<T: Clone + serde::Serialize> PretrainedSource<T> {
