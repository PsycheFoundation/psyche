...
mod tests {
    ...
    fn total_steps_test() {
        ...
        let mut c = Coordinator::<TestNode>::zeroed();

        c.config.warmup_time = 0;
        c.config.cooldown_time = 0;
        c.config.max_round_train_time = 0;
        c.config.round_witness_time = 0;
        c.config.rounds_per_epoch = rounds_per_epoch;
        c.config.init_min_clients = 2;
        c.config.min_clients = 2;
        c.config.witness_nodes = 1;
        c.config.global_batch_size_start = 1;
        c.config.global_batch_size_end = 1;
        c.config.global_batch_size_warmup_tokens = 0;
        c.config.verification_percent = 0;
        c.config.total_steps = total_steps;

        c.change_state(0, RunState::WaitingForMembers);

        let clients = [TestNode(1), TestNode(2)];
        let mut maybe_clients = Some(clients.iter());

        let mut ts = WAITING_FOR_MEMBERS_EXTRA_SECONDS + 1;
        let _ = c
            .tick(maybe_clients.take(), ts, 42)
            .expect("tick should succeed");
        assert!(matches!(c.run_state, RunState::Warmup));

        let mut safety = 0;
        while !matches!(c.run_state, RunState::Finished) {
            safety += 1;
            ...
            if matches!(c.run_state, RunState::RoundWitness) {
                ...
                round.witnesses.push(Witness::default()).unwrap();
            }

            let _ = c.tick(maybe_clients.take(), ts, 1337);
        }

        assert_eq!(c.progress.step - 1, total_steps);
        assert_eq!(c.progress.step, total_steps + 1);
    }
}
